...
1. まず、URL が必要になるので、コンテンツがオンラインでホストされていることを確認する必要があります。 (写真を altvr.com にアップロードしたり、ビデオ .mp4 ファイルをオンラインでホストしたり、Dlive ライブストリームリンクを使用したりすることができます。 https://dlive.tv/yourlivestream) 
...
* **管理者特権** を持つユーザー-最初にマルチメディアコンソールが構成されている領域に、モデレーターまたはホストロールがあるユーザー
* **すべてのユーザー** -すべてのユーザー

これらのロールは、この一覧で選択されたロールの上にあるすべてのロールにも、その機能を使用するためのアクセス許可が付与されるという意味でスタックします。  例: **管理者特権** を持つユーザーは、altているモデレーターまたはホスト * * ではなく、 **所有者** を含めます。 ロールの割り当てによって制御される機能を次に示します。
...
> [!NOTE]
> 必須: ビデオの長さと一致する時間を指定して、ビデオを適切に転送できるようにします。 たとえば、ビデオの長さが5分300秒の場合、ビデオは次のコンテンツにスキップされません。
...
メディアリストは、ロールや再生設定のような JSON 構造のルートで設定されたプロパティです。  これは、次のいずれかのメディア構成構造を含むことができる単純な配列です。 (それぞれの機能の詳細については、上記のプロパティの説明を参照してください)。
